  ...
  renderRoute = (route: IRouteParams) => {
    ...
    switch (type) {
      case RouteType.AUTH: {
        if (currentUser) {
          return (
            <Redirect exact from={path} key={`${path}->${RoutePath.HOME}`} to={generateRoutePath(RoutePath.HOME, {})} />
          )
        }
...
const routes: IRouteParams[] = [
  ...
  {
    Component: lazyWP(() => import('../pages/solution-list')),
    path: RoutePath.SOLUTION_LIST,
    type: RouteType.PRIVATE,
  },
  {
    Component: lazyWP(() => import('../pages/solution-details-page')),
    path: RoutePath.SOLUTION_DETAILS,
    type: RouteType.PRIVATE,
  },
  {
